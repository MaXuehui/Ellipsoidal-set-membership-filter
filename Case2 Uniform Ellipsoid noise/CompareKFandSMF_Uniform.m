...
%%%%%%%%  Uniform Noise  %%%%%%%%%%%%%
load NoiseUniformData3dim.mat
NoiseUniformData3dim = random_ellips_norm3;
load NoiseUniformData2dim.mat
NoiseUniformData2dim = random_ellips_norm2;

N = length(NoiseUniformData2dim);
...
x(:,1) = [0; 0; 0];
y(:,1) = H*x(:,1)+NoiseUniformData2dim(:,1);
y_real(:,1) = H*x(:,1);

for i = 1:N-1
    x(:,i+1) = F*x(:,i)+NoiseUniformData3dim(:,i);
    y(:,i+1) = H*x(:,i+1)+NoiseUniformData2dim(:,i);
    y_real(:,i+1) = H*x(:,i+1);
end
