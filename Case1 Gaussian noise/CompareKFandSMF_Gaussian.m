...
%%%%%%%%  Gaussian Noise  %%%%%%%%%%%%%
load NoiseGaussianData3dim_99pc.mat
NoiseGaussianData3dim = random_noise_data;
load NoiseGaussianData2dim_99pc.mat
NoiseGaussianData2dim = random_noise_data;

N = length(NoiseGaussianData2dim);
...
x(:,1) = [0; 0; 0];
y(:,1) = H*x(:,1)+NoiseGaussianData2dim(:,1);
y_real(:,1) = H*x(:,1);

for i = 1:N-1
    x(:,i+1) = F*x(:,i)+NoiseGaussianData3dim(:,i);
    y(:,i+1) = H*x(:,i+1)+NoiseGaussianData2dim(:,i);
    y_real(:,i+1) = H*x(:,i+1);
end
...
%%%%%%%%%  set membership filter (minimal Volume) %%%%%%%%%
P(:,:,1) = [10 0 0; 0 10 0; 0 0 10];
x_hat(:,1) = [0; 0; 0];
y_hat(:,1) = H*x_hat(:,1);
...
for i = 1:N-1
    ...
    if criteria > 0
        ...
        eigenvalue = eig(P(:,:,i+1)*H'*inv(R)*H);
        ...
        eqn = eigenvalue(1)/(1+q_solve*eigenvalue(1))+eigenvalue(2)/(1+q_solve*eigenvalue(2)) == n*beta_d/beta;
